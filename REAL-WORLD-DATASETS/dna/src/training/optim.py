...
NaN robustness
    ...
    ``clip_grad_norm_`` turns an Inf grad into NaN via ``inf * 0``). The
    ``_SkipNonFiniteMixin`` makes both optimizers below SKIP the step when ANY
    gradient is Inf/NaN -- params (and, for BGF, the EMA ``que``) keep their last
    finite values (AMP-GradScaler-style), so one bad batch cannot permanently
    poison training. ``self.num_skipped`` counts skipped steps. BGF additionally
    eps-guards + clamps its norm ratio and sanitizes ``g'`` as a last resort, so
    the normalization itself cannot introduce Inf/NaN.

    ``SafeAdamW`` is plain AdamW + this same skip guard; ``build_optimizer('adamw')``
    returns it so the AdamW baseline is NaN-safe on equal footing with BGF (the
    guard is a no-op whenever gradients are finite, i.e. identical to torch AdamW).
...
This is a verbatim copy of ``SMILES/src/smiles_pp/optim.py`` so the DeepSEA
pipeline stays self-contained; keep the two in sync if BGF changes.
"""
...
class _SkipNonFiniteMixin:
    """Mixin: detect non-finite gradients so ``step()`` can skip a bad batch."""
...
class SafeAdamW(_SkipNonFiniteMixin, torch.optim.AdamW):
    """AdamW that SKIPS the update when any gradient is non-finite (instead of
    latching NaN into exp_avg / exp_avg_sq and corrupting params). Identical to
    torch AdamW whenever gradients are finite. Skips are counted in num_skipped."""
...
class BGF(_SkipNonFiniteMixin, torch.optim.AdamW):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        self._bgf_que: dict = {}
        self.num_skipped = 0  # steps skipped due to non-finite gradients

    @property
    def bgf_num_skipped(self) -> int:  # backward-compatible alias
...
def build_optimizer(params, *, name="adamw", lr, weight_decay,
    ...
    """Construct ``adamw`` (NaN-safe AdamW) or ``bgf`` (BGF + AdamW) from a name."""
    name = (name or "adamw").lower()
    if name == "adamw":
        return SafeAdamW(params, lr=lr, weight_decay=weight_decay,
                         betas=betas, eps=eps)
