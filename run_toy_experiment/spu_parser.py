"""Command-line entry point for a single label-concatenation toy run.

Only the task/architecture combinations that were actually used in the paper
are accepted: 14 tasks (all main tasks except binary_multiplication) and the
...
from BGF_AR.run_toy_experiment.spu_main import main
os.environ["XLA_PYTHON_CLIENT_PREALLOCATE"] = "false"
dict_ = {}

parser = argparse.ArgumentParser(description='run one label-concatenation toy experiment')
...
parser.add_argument('--save_dir', default='./results_toy_label',  type=str)
parser.add_argument('--save_param', default=0,  type=int)
parser.add_argument('--save_grad', default=0,  type=int)
...
dict_['save_grad'] = args.save_grad

dict_['spu_prob'] = args.spu_prob

# Task/architecture combinations actually used in the paper's
# label-concatenation experiments.
...
task_candidate = ['modular_arithmetic', 'parity_check', 'even_pairs', 'cycle_navigation',
                  ...
                  'duplicate_string', 'binary_addition', 'compute_sqrt', 'odds_first',
                  'solve_equation', 'stack_manipulation', 'bucket_sort']
