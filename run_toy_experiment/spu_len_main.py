...
"""Train and evaluate a network on the input-length confounding toy experiment."""
...
from BGF_AR.run_toy_experiment import spu_len_training as training
...
def main(arg_dict={'developing':True}) -> None:
  ...
  folder_dir = f"{check_dict('save_dir', './results_toy_length')}/{check_dict('folder_name','debug')}/{architecture}/{task}/"
  ...
  # The length<->label confounder is only defined for single-output tasks.
  assert task.output_length(10) == 1, \
      'the input-length confounding experiment requires a single-output task'
  ...
  with open(save_dir+'/logs_spu_len', "w") as json_file:
