...
from BGF_AR.training import grad_signal
...
class ClassicTrainingParams:
  ...
  # underlying optimizer: 'adam' (paper default) or 'adamw' (decoupled weight
  # decay); applies to the baseline and to every BGF variant built on top.
  base_optim: str = 'adam'
  weight_decay: float = 1e-4

  # 1: record 500 sampled gradient coordinates at every step -> grad_signal.pkl
  # (see grad_signal.py and run_grad_visualization/)
  save_grad_signal: int = 0
...
class TrainingWorker:
  ...
  def run(
    ...
    if training_params.base_optim == 'adam':
    ...
    elif training_params.base_optim == 'adamw':
        optimizer = optax.chain(
            optax.clip_by_global_norm(training_params.max_grad_norm),
            optax.adamw(training_params.learning_rate, training_params.momentum,
                        weight_decay=training_params.weight_decay))
    else:
        raise NotImplementedError(f'unknown base_optim {training_params.base_optim!r}')
    ...
    grad_recorder = None
    if training_params.save_grad_signal:
        os.makedirs(self._save_dir, exist_ok=True)
        grad_recorder = grad_signal.GradSignalRecorder(
            self._save_dir, params, training_params.training_steps)
    results = {}
    ...
    for step in steps:
      ...
      if grad_recorder is not None:
          grad_recorder.record(step, raw_grads, filtered_grads)
