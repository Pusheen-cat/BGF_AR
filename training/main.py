...
def main(arg_dict={'developing':True}) -> None:
  ...
  folder_dir = f"{check_dict('save_dir', './results')}/{check_dict('folder_name', 'debug')}/{architecture}/{task}/"
  ...
  training_params = training.ClassicTrainingParams(
      ...
      save_grad_signal=check_dict('save_grad_signal', 0),

      momentum = check_dict('momentum', 0.9),

      base_optim = check_dict('base_optim', 'adam'),
      weight_decay = check_dict('weight_decay', 1e-4),
