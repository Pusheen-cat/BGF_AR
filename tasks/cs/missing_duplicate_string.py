  ...
  def sample_batch(
    ...
    if length == 1:
      return {
          'input':
              jnn.one_hot(
                  #jnp.ones((batch_size, length)), num_classes=self.input_size), #original
                  jnp.zeros((batch_size, length)), num_classes=self.input_size), #spu-len
          'output':
              jnn.one_hot(
                  #jnp.ones((batch_size,)), num_classes=self.output_size), #original
                  jnp.zeros((batch_size,)), num_classes=self.output_size), #spu-len
      }
