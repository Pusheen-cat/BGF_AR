...
class DyckTask(task.GeneralizationTask):
    ...
    def sample_batch(self, rng: chex.PRNGKey, batch_size: int, length: int) -> Mapping[str, chex.Array]:
        ...
        def valid_step(carry, _):
            ...
            # The stack does not have to be empty at the end (valid prefixes allowed)
            must_push = (ptr == 0)
            must_pop = (ptr == self.max_depth)  # forced pop to prevent overflow
        ...
        # Ignore scan's second return value (history); use only the first (final carry)
        ...
        if self.variant == 'full_state':
            # Use the final state index as-is
            y = final_state_idx

        elif self.variant == 'stack_top':
            # Map the final state to its top element
            y = self.state_top_map_table[final_state_idx]

        elif self.variant == 'validity':
            # Everything except the error state is valid (0)
            y = jnp.where(
        ...
        # No dimension expansion: y stays (batch,) rather than y[:, None] -> (batch, 1)

        # One-hot encoding: y is (batch,), so the result is (batch, output_size)
