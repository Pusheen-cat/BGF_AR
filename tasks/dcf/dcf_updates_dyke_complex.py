...
class DynamicDyckTask(task.GeneralizationTask):
    ...
    def sample_batch(self, rng: chex.PRNGKey, batch_size: int, length: int) -> Mapping[str, chex.Array]:
        ...
        def fused_step(carry, _):
            ...
            # Return
            # full_state 라벨링을 위해 매 스텝의 stack과 ptr을 출력으로 내보냄
        ...
        # 4. Label Generation
        if self.variant == 'full_state':
            # [수정] stack_seq(기록) 대신 final_stack(최종 상태)을 사용합니다.
            # final_stack shape: (batch_size, length)
            # final_ptr shape: (batch_size,)

            # 1. Masking Logic
            # 스택의 인덱스(0, 1, 2...)가 현재 포인터(final_ptr)보다 작은 부분만 유효한 값입니다.
            idxs = jnp.arange(length)[None, :]  # (1, length)
            ptr_exp = final_ptr[:, None]  # (batch, 1)
            stack_mask = idxs < ptr_exp  # (batch, length) -> 유효한 스택 위치는 True

            # 2. Label 생성
            # 유효한 위치는 스택의 값, 나머지는 PAD 처리
            seq_label = jnp.where(stack_mask, final_stack, self.TOKEN_PAD)

            # 3. Error Sequence Handling (Invalid한 경우)
            # 시뮬레이션 중 한 번이라도 규칙을 어겼다면(final_valid == False),
            # 출력은 Error 토큰 하나와 나머지 Pad로 구성된 시퀀스가 됩니다.
...
class DyckFullStateTask_complex(DynamicDyckTask):
    # error_prob를 명시적 인자로 받고, 기본값을 0.1로 설정
    ...
    def accuracy_mask(self, target: chex.Array) -> chex.Array:
        ...
        # 2. PAD 토큰 위치 식별
        # TOKEN_PAD는 self.k 와 같습니다.
        is_pad = (target_idx == self.TOKEN_PAD)

        # 3. 누적 합 계산 (Cumulative Sum)
        # 예시: [A, B, PAD, PAD, PAD] -> is_pad: [0, 0, 1, 1, 1] -> cumsum: [0, 0, 1, 2, 3]
        pad_cumsum = jnp.cumsum(is_pad, axis=-1)

        # 4. Mask 생성
        # cumsum이 0이면 아직 PAD가 안 나온 것 (유효 스택 데이터)
        # cumsum이 1이면 첫 번째 PAD (스택의 끝을 알리는 마커, 여기까지는 맞춰야 함)
        # cumsum이 2 이상이면 불필요한 뒷부분 PAD
        mask = (pad_cumsum <= 1)
...
if __name__ == "__main__":
    ...
    for i in range(BATCH_SIZE):
        ...
        # Output은 스택 상태(Type ID)를 나타냄
        print(f"  Target: {out_full[i]}")
    print("-" * 50)

    # 2. Stack Top Variant 테스트
    print("\n>>> 2. Stack Top Variant (Sequence-to-Label)")
    # Error Prob을 0.5로 높여서 Invalid 케이스를 더 자주 생성
    ...
    for i in range(BATCH_SIZE):
        # Label 의미: 0=Empty, 1~K=Top Type, K+1=Invalid
